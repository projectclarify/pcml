...
from antidote.research.sst2 import models
from antidote.research.sst2 import input_pipeline
...
def compute_metrics(*, labels: Array, logits: Array) -> Metrics:
  ...
  return Metrics(loss=jnp.sum(loss),
...
def model_from_config(config: ml_collections.ConfigDict):
  ...
  model = models.TextClassifier(embedding_size=config.embedding_size,
...
def train_step(
  ...
  def loss_fn(params):
    ...
    logits = state.apply_fn(variables,
                            batch['token_ids'],
                            batch['length'],
                            deterministic=False,
                            rngs=rngs)
...
def normalize_batch_metrics(batch_metrics: Sequence[Metrics]) -> Metrics:
  ...
  return Metrics(loss=total_loss.item() / total,
                 accuracy=total_accuracy.item() / total)
...
def evaluate_model(eval_step_fn: Callable[..., Any],
                   state: TrainState,
                   batches: Union[Iterable[Example], tf.data.Dataset],
                   epoch: int,
                   rngs: Optional[Dict[str, Any]] = None) -> Metrics:
  ...
  logging.info('eval  epoch %03d loss %.4f accuracy %.2f', epoch, metrics.loss,
...
def train_epoch(
  ...
  logging.info('train epoch %03d loss %.4f accuracy %.2f', epoch, metrics.loss,
               metrics.accuracy * 100)
