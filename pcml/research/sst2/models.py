...
class Embedder(nn.Module):
  ...
  def setup(self):
    ...
    self.word_dropout_layer = WordDropout(dropout_rate=self.word_dropout_rate,
...
class SimpleLSTM(nn.Module):
  ...
  @functools.partial(nn.transforms.scan,
  ...
  def initialize_carry(batch_dims, hidden_size):
    ...
    return nn.OptimizedLSTMCell.initialize_carry(jax.random.PRNGKey(0),
...
class AttentionClassifier(nn.Module):
  ...
  def __call__(self,
    ...
    deterministic = nn.module.merge_param('deterministic', self.deterministic,
                                          deterministic)
    encoded_inputs = self.dropout_layer(encoded_inputs,
...
class TextClassifier(nn.Module):
  ...
  def setup(self):
    ...
    self.classifier = AttentionClassifier(hidden_size=self.hidden_size,
                                          output_size=self.output_size,
                                          dropout_rate=self.dropout_rate)

  def embed_token_ids(self,
                      token_ids: Array,
                      deterministic: Optional[bool] = None) -> Array:
    deterministic = nn.module.merge_param('deterministic', self.deterministic,
                                          deterministic)
  ...
  def __call__(self,
    ...
    embedded_inputs = self.embed_token_ids(token_ids,
                                           deterministic=deterministic)
    logits = self.logits_from_embedded_inputs(embedded_inputs,
                                              lengths,
                                              deterministic=deterministic)
    return logits
